...
std::vector<double> linspace2(double a, double b, size_t N) {
...
Parameters
x: The coordinate to integrate along. If None (default), use spacing dx between
consecutive elements in y. (not implemented yet)
...
std::vector<long double> cumtrapz(std::vector<double> x,
  ...
  double h = (x[n] - x[0]) / n;
  std::vector<long double> res(n, 0);
  res[0] = fx[0];
